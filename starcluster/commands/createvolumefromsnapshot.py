...
class CmdCreateVolumeFromSnapshot(CmdBase):
    """
    createvolumefromsnapshot [options] <volume_size> <volume_zone>

    Create a new EBS volume for use with StarCluster
    """

    names = ['createvolumefromsnapshot', 'cvs']
    ...
    def addopts(self, parser):
        ...
        parser.add_option(
            "-o", "--snapshot-id", dest="snapshot_id",
            action="store", type="string", default=None,
            help="The snapshot id to use")
    ...
    def execute(self, args):
        ...
        snapshot = self.opts.snapshot_id
        ...
        try:
            vc.create_from_snapshot(size, zone, snapshot, name=self.opts.name, tags=self.opts.tags)
